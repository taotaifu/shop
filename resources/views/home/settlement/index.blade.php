...
<div class="beij_center">
        ...
        <div class="hd-content jies_y_shouh_diz shiq_1">
            @if(count ($addresses)!=0)
                @foreach($addresses as $address)
                    <ul>
                        <li class=" @if($address['is_default'] == 1) cur @endif" address_id="{{$address['id']}}">
                            <div class="dangq_xuanz_diz">当前地址</div>
                            <span>{{$address['name']}}</span>
                            <span>{{$address['province']}}/{{$address['detail']}}</span>
                            <span>{{$address['phone']}}</span>
                            <div class="bianji_yv_shanc">
                                <a href="{{route ('home.address.show',$address)}}">设为默认</a>
                                <a href="#">编辑</a>
                            </div>
                        </li>
                    </ul>
                ...
                @foreach($settlements as $settlement)
                    <div class="goods_list_neik">
                        ...
                        <div class="goods_item">
                            <div class="p_img"><a href="#"><img src="{{$settlement['pic']}}"></a></div>
                            <div class="goods_msg">
                                <div class="p_name">
                                    <a href="#">{{$settlement['title']}} {{$settlement['spec']}}</a>
                                </div>
                                <div class="p_price">
                                    <span class="jq_price">￥ {{$settlement['price']}}</span>
                                    <span>x{{$settlement['num']}}</span>
                                    <span>有货</span>
                                </div>
                            </div>
                        </div>
    ...
    <div class="order_summary">
        <div class="qianq_mx">
            <div class="jiaq_meih">
                <span class="xiangq_leib"><em class="goumai_ges">{{count($settlements)}}</em> 件商品，总商品金额：</span>
    ...
    <div class="trade_foot_detail_com">
        ...
        <div class="zuiz_diz">
            @if($defaultAddress)
                <span>寄送至： {{$defaultAddress['province']}}/{{$defaultAddress['detail']}}</span>
                </br>
...
<script>
    function send() {
        //检测是否有选中地址
        // alert(1);
        let len = $('.hd-content').find('li.cur').length;
        if(len ==0){
            layer.msg('请先选择提交地址');
            return ;
        }
        // alert(1);
        //提交订单
        $.post("{{route('home.settlement.store')}}", {
            address_id: $('.hd-content').find('li.cur').attr('address_id'),
